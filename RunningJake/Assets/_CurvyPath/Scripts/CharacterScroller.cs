    ...
    public class CharacterScroller : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int i = 0; i < CharacterManager.Instance.characters.Length; i++)
            {
                ...
                if (character.GetComponent<PlayerBallController>()!=null)
                {
                    Component ballController = character.GetComponent<PlayerBallController>();
                ...
                character.transform.GetChild(1).gameObject.layer = LayerMask.NameToLayer("CharacterSelectionUI");
            }
